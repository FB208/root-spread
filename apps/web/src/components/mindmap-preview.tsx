...
function PreviewNode({ accent, delay, title, meta, value }: PreviewNodeProps) {
  return (
    <div
      className="preview-node min-w-[220px] rounded-2xl border border-white/10 bg-[linear-gradient(180deg,rgba(8,13,30,0.96),rgba(12,18,38,0.92))] px-4 py-3 text-left shadow-[0_20px_60px_rgba(0,0,0,0.35)]"
      style={delay ? { animationDelay: delay } : undefined}
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        <span
          className="preview-node-dot h-2.5 w-2.5 rounded-full"
      ...
      <div className="space-y-1">
        <p className="text-xs uppercase tracking-[0.24em] text-white/35">Node</p>
        <h3 className="text-sm font-semibold text-white/90">{title}</h3>
        <p className="text-xs text-[#9ba9cf]">{value}</p>
      </div>
    </div>
  );
}
...
const nodes: Node[] = [
  {
    ...
    style: { background: "transparent", border: "none", width: 252 },
  },
  {
    id: "auth",
    position: { x: 610, y: 26 },
...
export function MindmapPreview() {
  return (
    <ReactivePanel
      className="panel preview-panel relative h-[440px] overflow-hidden rounded-[28px] p-0 breathe-card"
      rotationLimit={5}
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between border-b border-white/[0.08] bg-[linear-gradient(180deg,rgba(5,8,20,0.96),rgba(5,8,20,0.88),rgba(5,8,20,0.62))] px-5 py-4 backdrop-blur-md">
        <div>
        ...
        <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-white/35">
          <span className="rounded-full border border-white/10 px-2 py-1">mind map</span>
          <span className="rounded-full border border-white/10 px-2 py-1">table</span>
        </div>
      </div>
      ...
      <div className="preview-flow-shell absolute inset-x-0 top-[20px] bottom-[-20px]">
        <ReactFlow
          nodes={nodes}
          edges={edges}
          fitView

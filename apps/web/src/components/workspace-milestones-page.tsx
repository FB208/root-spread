    ...
    <div className="space-y-3">
      <section className="panel rounded-[18px] px-4 py-3.5">
        <div className="compact-page-header">
          <div>
            <p className="compact-kicker">Milestone Console</p>
            <h2 className="compact-title">管理阶段归档与历史快照</h2>
            <p className="compact-copy">
              把里程碑抽成独立页面后，任务区只负责执行，阶段归档、历史追溯和时间节点统一在这里管理。
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="compact-chip">里程碑 {milestones.length}</span>
            <Link
      ...
      <section className="grid gap-3 xl:grid-cols-[0.95fr_1.05fr]">
        <div className="panel rounded-[18px] p-4">
          <p className="compact-kicker">Create Milestone</p>
          <h3 className="compact-card-title">创建新的阶段归档</h3>
          <p className="compact-card-copy">
            设定阶段名称、说明和目标时间，归档后可以直接从任务工作台切换到对应快照视图。
          </p>

          <form className="mt-3.5 space-y-2.5" onSubmit={handleCreateMilestone}>
            <div>
        ...
        <div className="panel rounded-[18px] p-4">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="compact-kicker">Snapshots</p>
              <h3 className="compact-card-title">已创建的里程碑</h3>
            </div>
            <p className="text-xs text-text-muted">点击任意里程碑可直接进入任务工作台并查看该快照。</p>
          </div>

          <div className="mt-3 space-y-2.5">
            <Link
              className="compact-list-card block transition hover:border-white/[0.18] hover:bg-white/[0.05]"
              href={`/workspaces/${workspaceId}/tasks`}
            >
              <p className="text-sm font-semibold text-white/88">当前任务视图</p>
              <p className="mt-1.5 text-[13px] text-text-muted">回到实时任务树，继续处理未归档节点。</p>
            </Link>

            {sortedMilestones.length ? (
              sortedMilestones.map((milestone) => (
                <article key={milestone.id} className="compact-list-card">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <p className="text-[15px] font-semibold text-white/90">{milestone.name}</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-white/50">
                    <span className="compact-chip">归档任务 {milestone.archived_task_count} 项</span>
                    <span className="compact-chip">目标时间 {new Date(milestone.target_at).toLocaleString("zh-CN")}</span>
                    <span className="compact-chip">创建时间 {new Date(milestone.created_at).toLocaleString("zh-CN")}</span>
                  </div>
                </article>
              ))
            ) : (
